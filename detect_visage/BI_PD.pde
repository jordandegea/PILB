...
void setup() {
  ...
  Capture.list();
  cap = new Capture(this, Capture.list()[0]);
  cap.start();
...
void draw() {
  ...
  hog.detectMultiScale(m3, found, weight, 1, new Size(8, 8), new Size(32, 32), 1.05, 2, false);
  ...
  if (rects.length > 0) {
    for (int i=0; i<rects.length; i++) {
      ...
      ellipse(rects[i].x/ratio + (rects[i].width/ratio / 2), rects[i].y/ratio + (rects[i].height/ratio / 2), 12, 12);
      
      float printX = rects[i].x/ratio + (rects[i].width/ratio / 2) - X/2;
      float printY = rects[i].y/ratio + (rects[i].height/ratio / 2) - Y/2;
      
      println(printX + " - " + printY );
      
      text( printX + " - " + printY , 200, 20 );
    }
  }
